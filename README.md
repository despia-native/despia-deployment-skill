...
**Claude Code** — drop the folder in your skills directory and it loads on the next session.

```bash
git clone https://github.com/despia-native/despia-deployment-skill.git
mkdir -p ~/.claude/skills
cp -r despia-deployment-skill/despia-store-provisioning ~/.claude/skills/
```

Use `.claude/skills/` inside a project instead of `~/.claude/skills/` if you only want it for that project.

**Claude apps** — zip the `despia-store-provisioning` folder and upload it under Settings → Capabilities → Skills.

**Anything else that reads agent skills** — point it at `despia-store-provisioning/SKILL.md`. The reference files load themselves as they're needed, so nothing else to configure.
